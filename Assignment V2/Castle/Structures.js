...
function createPins1(number) {
  ...
  function createPins2(number) {
    ...
    for (var i = 0; i < WallX.length; i++) {
        ...
        let pos = {x:hello + 100, y: 2*height + 2, z: 2*width - 5};
        let scale = {x: 2, y: 2, z: 2};
        let quat = {x: 0, y: 0, z: 0, w: 1};
        let mass = 1;
    
        //threeJS Section
        let block = new THREE.Mesh(new THREE.BoxBufferGeometry(), new THREE.MeshPhongMaterial({color: 0x0036FF}));
